...
def test_derive_display_state_maps_terminal_and_waiting_states(app_modules):
    ...
    assert (
        model.derive_display_state(_job(readiness="waiting_dependency", depends_on="x"))
        == "waiting"
    )
    assert model.derive_display_state(_job(readiness="blocked", depends_on="x")) == "blocked"
    assert (
        model.derive_display_state(
            _job(execution="success", last_run_at="2026-04-17T10:45:00+0100")
        )
        == "completed"
    )
    assert (
        model.derive_display_state(_job(execution="failed", last_run_at="2026-04-17T10:45:00+0100"))
        == "failed"
    )
...
def test_status_and_scheduler_labels_are_clear(app_modules):
    model = app_modules.state_model
    assert (
        model.display_label(_job(execution="success", last_run_at="2026-04-17T10:45:00+0100"))
        == "Completed"
    )
